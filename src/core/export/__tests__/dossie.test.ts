...
import { dossieParaIA, nomeDoArquivoDoDossie } from '../dossie';
...
describe('o dossiê diz o que o personagem TEM — nunca o que ele não tem', () => {
  ...
  it('modo PD: mostra PD e não mostra PE nem SAN', () => {
    const md = dossieParaIA(combatente({ usarPd: true, pd: { atual: 7, max: 12 } }));
  ...
  it('cada seção é separada por linha em branco, inclusive Ataques → Proteções', () => {
    const md = dossieParaIA(combatente());
    expect(md).toContain('\n\n## Proteções\n');
    expect(md).toContain('\n\n## Ataques\n');
  });

  it('condições ativas entram nos recursos e nas penalidades de perícia', () => {
    const md = dossieParaIA(combatente({ efeitosAtivos: ['Caído'] }));
  ...
  it('marcas de perda permanente e pendências ficam registradas quando existem', () => {
    ...
    expect(dossieParaIA(combatente({ poderesClassePendentes: 2 }))).toContain('2 poderes de classe');
  });
  ...
  it('Sobrevivente mostra estágio, não NEX; e o nome de arquivo é seguro', () => {
    ...
    expect(dossieParaIA(s)).toMatch(/^Sobrevivente · Estágio 3 · Origem:/m);
    expect(nomeDoArquivoDoDossie(s)).toBe('ze-do-acougue-dossie.md');
  });
