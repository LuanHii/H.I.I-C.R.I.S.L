  ...
  return (
    <div className="flex flex-col bg-ordem-ooze/90 rounded-lg p-3 border border-ordem-border relative overflow-hidden group min-h-[85px] justify-between">
      ...
      <div className="flex items-center gap-2 mb-1 opacity-80 z-10">
        ...
        <span className="text-xs font-bold uppercase tracking-wider text-ordem-text-secondary">{label}</span>
      </div>
      ...
      <div className="flex items-baseline gap-1 mt-auto z-10">
        ...
        <span className="text-sm font-mono text-ordem-text-muted font-semibold">/{max}</span>
      </div>
    </div>
  );
};
        ...
        <div className="bg-ordem-ooze border-b border-ordem-border p-4 pb-3 flex justify-between items-center gap-3">
          <div className="overflow-hidden mr-2">
            ...
            <div className="flex items-center gap-2 text-xs font-mono mt-1">
              ...
              <span className="text-ordem-text-secondary font-semibold">{agent.classe === 'Sobrevivente' ? `Estágio ${agent.estagio ?? 1}` : `${agent.nex}% NEX`}</span>
              {hasPendencias && (
                <>
                  <span className="text-ordem-text-muted">|</span>
          ...
          <div className="border-t border-ordem-border bg-ordem-black-deep/60 p-3 space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-ordem-black/40 border border-ordem-border rounded p-2">
                <div className="text-[10px] text-ordem-text-muted uppercase tracking-widest">PV</div>
                <div className="text-sm font-bold text-white font-mono">{agent.pv.atual}/{agent.pv.max}</div>
              </div>
              {usarDeterminacao ? (
                <div className="bg-ordem-black/40 border border-ordem-border rounded p-2 col-span-2">
                  <div className="text-[10px] text-ordem-text-muted uppercase tracking-widest">PD</div>
                  <div className="text-sm font-bold text-white font-mono">{agent.pd?.atual ?? 0}/{agent.pd?.max ?? 0}</div>
                </div>
              ) : (
                <>
                  <div className="bg-ordem-black/40 border border-ordem-border rounded p-2">
                    <div className="text-[10px] text-ordem-text-muted uppercase tracking-widest">PE</div>
                    <div className="text-sm font-bold text-white font-mono">{agent.pe.atual}/{agent.pe.max}</div>
                  </div>
                  <div className="bg-ordem-black/40 border border-ordem-border rounded p-2">
                    <div className="text-[10px] text-ordem-text-muted uppercase tracking-widest">SAN</div>
