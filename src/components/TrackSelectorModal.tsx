...
export const TrackSelectorModal: React.FC<TrackSelectorModalProps> = ({ agent, onConfirm, onDefer }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ordem-black/90 backdrop-blur-sm p-4">
      <div className="bg-ordem-ooze border border-ordem-border-light rounded-xl p-6 max-w-4xl w-full shadow-2xl h-[80vh] flex flex-col">
        <div className="mb-4 border-b border-ordem-border pb-4">
            <h2 className="text-2xl font-serif text-ordem-red mb-1">ESCOLHA SUA TRILHA</h2>
            <p className="text-ordem-text-secondary">
            ...
            {availableTracks.map(track => (
                ...
                >
                    <div className="flex justify-between items-start">
                        <h3 className={`font-bold text-lg ${selectedTrack === track.nome ? 'text-ordem-red' : 'text-white group-hover:text-ordem-white'}`}>
                            {track.nome}
                        </h3>
                        {selectedTrack === track.nome && (
                    ...
                    <div className="mt-4 pt-4 border-t border-ordem-border/50">
                        <span className="text-xs text-ordem-text-secondary uppercase tracking-wider font-bold mb-2 block">
                            Primeira Habilidade
                        </span>
                        {track.habilidades[0] && (
                            <div>
                                <span className="text-ordem-white-muted font-bold text-sm block mb-1">
                                    {track.habilidades[0].nome}
                                </span>
                                <p className="text-xs text-ordem-text-secondary line-clamp-3">
        ...
        <div className="mt-6 pt-4 border-t border-ordem-border flex justify-end gap-4 items-center">
            <div className="text-ordem-text-secondary text-sm italic">
