  ...
  return (
    <Modal open={aberto} onOpenChange={(v) => { if (!v) onFechar(); }}>
      <ModalContent
        ...
        className={`w-full border-0 bg-transparent p-0 shadow-none ${largura === 'md' ? 'max-w-md' : 'max-w-2xl'}`}
      >
        <div data-classe={classe} className="relative flex max-h-[85vh] flex-col overflow-hidden border border-white/10 bg-[var(--mestre-superficie,#16161a)] shadow-[0_1px_0_0_rgba(255,255,255,0.05)_inset,0_40px_80px_-30px_rgba(0,0,0,1)]">
          <Cantos />
          ...
          <header className="relative flex items-start justify-between gap-4 px-6 pt-5">
            <div className="min-w-0">
              ...
              <ModalTitle className="mt-1 font-display text-2xl uppercase leading-none tracking-[0.06em] text-white">
                {titulo}
              </ModalTitle>
            </div>
            <button
          ...
          {(busca || filtros) && (
            <div className="relative space-y-2 px-6 pt-4">
              {busca && (
          ...
          {rodape && (
            <footer className="relative flex items-center justify-between gap-3 border-t border-white/10 px-6 py-3">
              {rodape}
            </footer>
          )}
        </div>
